...
# left bound of the obstacle movement in x-direction
...
if rospy.has_param("pos_rb"):
  pos_rb = rospy.get_param("pos_rb")
else:
  pos_rb = 24
...
def callback_base_pose_ground_truth(base_pose_ground_truth):
  ...
  # Turn in x-direction
  if base_pose_ground_truth.pose.pose.position.x >= pos_rb:
    Twist_msg.linear.x = -vel_x
  if base_pose_ground_truth.pose.pose.position.x <= pos_lb:
    Twist_msg.linear.x = vel_x
